...
RUN dnf clean all
RUN dnf update
RUN dnf config-manager -y --add-repo https://packages.microsoft.com/config/rhel/9/prod.repo
...
RUN runuser -l ${USERNAME} -c "pip3 install ansible ansible-navigator ansible-lint yamllint datadog flake8 black"
